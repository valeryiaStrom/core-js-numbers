...
function toNumber(value, def) {
  ...
  if (Number.isNaN(Number(value))) {
    conversionResult = def;
  } else {
    conversionResult = Number(value);
  }
...
function getSumOfDigits(num) {
  const numberSplitted = String(num).split('');
  const numbersArray = numberSplitted.map((numberStr) => +numberStr);
  return numbersArray.reduce((acc, i) => acc + i, 0);
}
